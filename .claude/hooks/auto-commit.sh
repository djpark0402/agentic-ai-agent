...
if git commit -m "$COMMIT_MSG" >/tmp/claude-auto-commit.log 2>&1; then
  ...
  # 세션 브랜치(feat/new-promt-*)에서 작업 중이면 머지 알림 포함
  if echo "$BRANCH" | grep -q '^feat/new-promt-'; then
    jq -n --arg branch "$BRANCH" --arg sha "$SHA" '{
      "systemMessage": ("✅ 세션 브랜치(" + $branch + ")에 로컬 자동 커밋 완료 (" + $sha + ")"),
      "hookSpecificOutput": {
        "hookEventName": "Stop",
        "additionalContext": ("📌 현재 세션 브랜치(" + $branch + ")에서 작업 중입니다. 사용자에게 feat/pr-develop에 머지할까요? 라고 반드시 물어보세요.")
      }
    }'
  else
    jq -n --arg branch "$BRANCH" --arg sha "$SHA" '{
